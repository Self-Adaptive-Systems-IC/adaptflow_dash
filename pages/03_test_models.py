...
def test_model(model_name):
    ...
    for seconds in range(0,2000):
        with placeholder_data_visualization.container():
            ...
            new_data = generate_new_param(features) # Gera um novo conjunto de dados para teste
            
            # Para cada modelo vai realizar a consuta na respectiva api
            info = {}
            for model in model_names:
                info[model] = requests_2_apis(model, elapsed_time, new_data, historical_data_df)
            ...
            for model, model_infos in info.items():
                historical_data.append(
                    {"model": model, "accuracy": model_infos["acc"], "time": elapsed_time}
                )
            all_results = pd.concat([all_results, pd.DataFrame(historical_data)], ignore_index=True)
            ...
            all_model_op_results = all_results[all_results['model'] == option] # Filtra os resultados historicos do modelo
            model_acc_mean = all_model_op_results['accuracy'].mean() # Calcula a média da acuracia do modelo
            ...
            with acc_title:
                st.metric(label="Accuracy", value=f"{round(filtered_info['acc'],2)}%", delta=f"{round((filtered_info['acc']-last_acc),2)}%")
...
with placeholder_form.container():
    with st.form("form1"):
        ...
        option = st.selectbox(
            "Modelo", options=(model_names), key="visibility"
        )
